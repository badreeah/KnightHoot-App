...
export default function AddDevice({ navigation, route }) {
    ...
    const handleDeviceAdded = async () => {
        ...
        deviceData.bluetooth_id = route.params?.selectedDevice?.bluetooth_id || null;
        ...
        if (error) {
            ...
            alert(`Failed to ${editingDevice ? 'update' : 'add'} device. Check RLS policies.`);
        } else {
            await fetchDevices();
    ...
    const toggleDevice = async (deviceId, currentState) => {
        ...
        if (error) {
            ...
            setDevices(prev =>
                prev.map(d => (d.id === deviceId ? { ...d, enabled: !currentState } : d))
            );
        }
    };
    ...
    console.log('Filtered devices:', filteredDevices.length, 'from', devices.length);
    ...
    if (showAddForm) {
        ...
        return (
            <KeyboardAvoidingView style={styles.absoluteContainer} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                <ScrollView
                    style={{ flex: 1 }}
                ...
                >
                        ...
                        <View style={[styles.row, { marginTop: 16 }]}>
                            ...
                            >
                                <Text style={styles.selectTypeText}>
                                    {newDeviceType ? (newDeviceType === 'phone' ? 'iPhone' :
                                                    newDeviceType === 'laptop' ? 'Laptop' : 'Tablet') : 'Select Type'}
                                </Text>
                                <Ionicons name="chevron-down" size={16} color={COLORS.purple1} />
                            </TouchableOpacity>
                        </View>
                    </View>
                    ...
                    <TouchableOpacity style={styles.primaryButton} onPress={handleDeviceAdded}>
                        <Text style={styles.primaryButtonText}>{editingDevice ? 'Save Changes' : 'Add Now'}</Text>
                    </TouchableOpacity>
                </ScrollView>
    ...
    return (
        <KeyboardAvoidingView style={styles.absoluteContainer} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <ScrollView
                style={{ flex: 1 }}
                contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 200, flexGrow: 1 }}
                        ...
                        <View style={{marginTop: 20}}>
                            {filteredDevices.map(device => (
                                <View key={device.id} style={styles.card}>
                                    <TouchableOpacity onPress={() => handleDeleteDevice(device.id)} style={styles.deleteButton}>
                                    ...
                                    <View style={[styles.row, { marginBottom: 16 }]}>
                                        ...
                                        <Switch
                                            ...
                                            onValueChange={() => toggleDevice(device.id, device.enabled)}
                                            trackColor={{ false: COLORS.gray1, true: COLORS.purple1 }}
                                            thumbColor={'#ffffff'}
                                        />
                                    </View>
                                    <View style={styles.divider} />
                                    <View style={[styles.row, { marginTop: 16, justifyContent: 'space-between', paddingHorizontal: 8 }]}>
            ...
            {filteredDevices.length > 0 && (
                <View style={styles.bottomButtonContainer}>
                    <TouchableOpacity style={[styles.primaryButton, styles.bottomPrimaryButton]} onPress={() => setShowAddForm(true)}>
